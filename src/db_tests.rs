...
mod tests {
    ...
    async fn setup_test_db() -> PgPool {
        ...
        let database_url = std::env::var("TEST_DATABASE_URL")
            .unwrap_or_else(|_| std::env::var("SUPABASE_DATABASE_URL").expect("SUPABASE_DATABASE_URL must be set for tests"));

        let pool = PgPool::connect(&database_url).await.unwrap();
        
        // Clean up any existing test data
        sqlx::query("DELETE FROM posting_assets").execute(&pool).await.unwrap();
        sqlx::query("DELETE FROM post_images").execute(&pool).await.unwrap();
        sqlx::query("DELETE FROM asset_folders").execute(&pool).await.unwrap();
        sqlx::query("DELETE FROM posts").execute(&pool).await.unwrap();
        sqlx::query("DELETE FROM assets").execute(&pool).await.unwrap();
        sqlx::query("DELETE FROM folders").execute(&pool).await.unwrap();
