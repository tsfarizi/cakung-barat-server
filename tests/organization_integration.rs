...
mod organization_integration_tests {
    ...
    async fn test_update_member() {
        ...
        assert_eq!(updated_member.level, 2);
    ...
    async fn test_cache_behavior() {
        ...
        let req1 = test::TestRequest::get().uri("/organization").to_request();

        let resp1 = test::call_service(&app, req1).await;
        assert!(resp1.status().is_success());

        // Second request should be a cache hit
        let req2 = test::TestRequest::get().uri("/organization").to_request();
        ...
        let req3 = test::TestRequest::get().uri("/organization").to_request();
